...
class TenderCancellationResourceTest(BaseTenderUAContentWebTest):
    ...
    def test_create_tender_cancellation(self):
        ...
        response = self.app.post_json('/tenders/{}/cancellations?acc_token={}'.format(
            self.tender_id, self.tender_token), {'data': {'reason': 'cancellation reason', 'status': 'active', 'reasonType': 'unsuccessful'}})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
        cancellation = response.json['data']
    ...
    def test_patch_tender_cancellation(self):
        ...
        self.assertTrue(cancellation_create_date < response.json['data']['date'])
...
class TenderLotCancellationResourceTest(BaseTenderUAContentWebTest):
    ...
    def test_create_tender_cancellation(self):
        ...
        self.assertIn('id', cancellation)
        self.assertIn('date', cancellation)
        self.assertTrue(cancellation_create_date < cancellation['date'])
        self.assertIn(cancellation['id'], response.headers['Location'])
    ...
    def test_patch_tender_cancellation(self):
        ...
        self.assertEqual(response.json['data']["status"], "active")
        self.assertEqual(response.json['data']["reason"], "cancellation reason")
        self.assertEqual(cancellation_activate_date, response.json['data']['date'])
